...
Page({
  ...
  async saveTrajectory() {
    ...
    if (this._positionArr && this._positionArr.length > 0) {
      ...
      const speed = (elapsed >= 5 / 3600) ? Number((totalDistance / elapsed).toFixed(2)) : 0;
      // 进行精度控制，确保距离值为两位小数
      const formattedDistance = Number(totalDistance.toFixed(2));
    ...
    try {
      ...
      // 再次确保保存到数据库的距离值也进行精度控制
      const formattedDistance = Number(this.data.distance.toFixed(2));
  ...
  async loadTrajectoryList() {
    try {
      ...
      if (totalStepsResult.data && totalStepsResult.data.length > 0) {
        // 确保总距离精确到两位小数
        const totalDistance = totalStepsResult.data[0].exerciseTotalKm || 0;
        const preciseTotalDistance = Number(Number(totalDistance).toFixed(2));
        totalExerciseDistance = preciseTotalDistance;
  ...
  async updateUserExerciseTotalKm(currentKm) {
    try {
      ...
      const db = wx.cloud.database();
      // 确保currentKm先进行精度处理
      const preciseCurrentKm = Number(Number(currentKm).toFixed(2));
      ...
      let newExerciseTotalKm = preciseCurrentKm; // 如果是第一次记录

      if (result.data && result.data.length > 0) {
        // 用户已有记录，累加运动轨迹公里数
        const existingRecord = result.data[0];
        // 确保两个操作数都精确到两位小数后再相加
        const existingKm = Number(Number(existingRecord.exerciseTotalKm || 0).toFixed(2));
        newExerciseTotalKm = Number((existingKm + preciseCurrentKm).toFixed(2));
