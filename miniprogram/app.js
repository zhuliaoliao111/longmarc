App({
 ...
 //欣加
  patchNodeJSModules() {
    ...
    // 拦截require调用，防止加载Node.js特定模块
    const originalRequire = require;
    require = function(id) {
      // 拦截文件系统模块
      if (id === 'fs' || id === 'path') {
        console.warn(`已拦截对 ${id} 模块的加载`);
        return {
          readFileSync: (path) => {
            console.warn(`已拦截文件读取: ${path}`);
            return '{}';
          },
          readFile: (path, callback) => {
            console.warn(`已拦截异步文件读取: ${path}`);
            if (callback) callback(new Error('小程序环境不支持文件系统'));
          },
          existsSync: (path) => false,
          writeFileSync: () => { console.warn('文件写入被拦截'); }
        };
      }
      
      // 拦截其他可能的问题模块
      if (id.includes('config') || id.endsWith('.json')) {
        console.warn(`已拦截配置文件加载: ${id}`);
        return {};
      }
